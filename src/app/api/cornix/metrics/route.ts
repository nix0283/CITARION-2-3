...
async function getCopyTradingMetrics(startDate: Date, exchange?: string | null) {
  try {
    // Get connected exchange accounts
    ...
    for (const account of accounts) {
      ...
      if (account.exchangeId === 'okx' || account.exchangeId === 'bitget') {
        try {
          const client = await getExchangeClient(account.exchangeId as any, {
          ...
          console.error(`[Cornix] Error getting copy trading data from ${account.exchangeId}:`, err);
        }
      }
    }
