...
interface SyncResult {
  accountId: string;
  newPositions?: Array<{ id: string; symbol: string; side: string; size: number }>;
  closedPositions?: Array<{ id: string; reason: string }>;
  updatedPositions?: Array<{ id: string; changes: string[] }>;
...
export async function POST(request: NextRequest) {
  try {
    ...
    let result: Record<string, SyncResult>;
    ...
    const summary = {
      totalNewPositions: Object.values(result).reduce(
        (sum: number, r: SyncResult) => sum + (r.newPositions?.length || 0), 
        0
      ),
      totalClosed: Object.values(result).reduce(
        (sum: number, r: SyncResult) => sum + (r.closedPositions?.length || 0), 
        0
      ),
      totalUpdated: Object.values(result).reduce(
        (sum: number, r: SyncResult) => sum + (r.updatedPositions?.length || 0), 
        0
      ),
      errors: Object.entries(result)
        .filter(([_, r]: [string, SyncResult]) => r.errors && r.errors.length > 0)
        .map(([id, r]: [string, SyncResult]) => ({ accountId: id, errors: r.errors })),
    };
