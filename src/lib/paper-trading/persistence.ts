...
export class PaperTradingPersistence {
    ...
    try {
      ...
      await db.paperPosition.upsert({
        ...
        update: {
          ...
          trailingStop,
          totalFundingPaid: position.totalFundingPaid || 0, // Track from position data
  ...
  private deserializeAccountV2(dbAccount: any): PaperAccount | null {
    try {
      ...
      return {
        ...
        equity: dbAccount.equity,
        availableMargin: dbAccount.availableMargin,
        maxEquity: dbAccount.maxEquity || dbAccount.equity, // Use DB tracked value
        positions,
        tradeHistory,
        equityCurve,
        ...
        realizedPnl: tradeHistory.length > 0 ? tradeHistory.reduce((sum, t) => sum + (t.pnl || 0), 0) : 0, // Calculate from trades
  ...
  private tradeFromDB(dbTrade: any): PaperTrade {
    return {
      ...
      avgEntryPrice: dbTrade.positionId ? (dbTrade.position?.entryPrice || 0) : 0, // Get from linked position
      totalSize: dbTrade.size,
      openedAt: dbTrade.timestamp,
      avgExitPrice: dbTrade.price,
      closedAt: dbTrade.timestamp,
      closeReason: (dbTrade.reason as PaperTrade['closeReason']) || "MANUAL", // Map from DB reason field
      pnl: dbTrade.pnl || 0,
      pnlPercent: 0,
      fees: dbTrade.fee,
