...
export class PaperTradingEngine {
  ...
  private calculateFullMetrics(account: PaperAccount): PaperTradingMetrics {
    ...
    const drawdowns = account.equityCurve
    ...
    // Time in drawdown
    const inDrawdownCount = account.equityCurve.filter(p => p.drawdownPercent > 0).length;
    const timeInDrawdown = account.equityCurve.length > 0 
      ? (inDrawdownCount / account.equityCurve.length) * 100 
      : 0;
    ...
    // Convert to days (assuming 1 point per minute)
    maxDrawdownDuration = maxDrawdownDuration / (24 * 60);

    // Weekly and monthly returns
    const weeks = Math.max(1, Math.floor(tradingDays / 7));
    const months = Math.max(1, Math.floor(tradingDays / 30));
    const avgWeeklyReturn = tradingDays >= 7 ? account.totalPnlPercent / weeks : account.totalPnlPercent;
    const avgMonthlyReturn = tradingDays >= 30 ? account.totalPnlPercent / months : account.totalPnlPercent;

    // Market exposure (percentage of time with open positions)
    const withPositions = account.equityCurve.filter(p => p.openPositions > 0).length;
    const marketExposure = account.equityCurve.length > 0 
      ? (withPositions / account.equityCurve.length) * 100 
      : 0;

    // Value at Risk 95% and Expected Shortfall
    const sortedReturns = [...returns].sort((a, b) => a - b);
    const var95Index = Math.floor(sortedReturns.length * 0.05);
    const var95 = sortedReturns.length > 0 
      ? -(sortedReturns[var95Index] || sortedReturns[0] || 0) * 100 
      : 0;
    
    // Expected Shortfall (average of worst 5% returns)
    const tailReturns = sortedReturns.slice(0, Math.max(1, Math.floor(sortedReturns.length * 0.05)));
