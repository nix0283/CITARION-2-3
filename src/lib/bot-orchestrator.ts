...
export type BotConfig = GridBotConfig | DCABotConfig | BBBotConfig | Record<string, unknown>;
...
export class BotOrchestrator {
  ...
  static async executeTrade(
    ...
    try {
      ...
      const orderConfig: ExchangeOrderConfig = {
        exchangeId: bot.exchangeId as ExchangeId,
        mode: bot.mode,
  ...
  private static async executeGridStrategy(bot: BotInstance, account: unknown): Promise<void> {
    ...
    void gridLevels; void symbol; // Suppress unused warnings
  }

  private static async executeDCAStrategy(bot: BotInstance, account: unknown): Promise<void> {
    const config = bot.config as DCABotConfig;
    // DCA strategy implementation
    void config; // Suppress unused warning
  }

  private static async executeBBStrategy(bot: BotInstance, account: unknown): Promise<void> {
    const config = bot.config as BBBotConfig;
