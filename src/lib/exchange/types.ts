...
// ==================== EARN / SAVINGS TYPES ====================
/**
 * Unified Earn/Savings Types
 * 
 * Intersection of earn products across ALL 5 exchanges:
 * - Binance: Simple Earn (Flexible, Locked)
 * - Bybit: Earn (Flexible, Locked)
 * - OKX: Finance/Savings (Flexible, Fixed)
 * - Bitget: Earn (Savings, Fixed)
 * - BingX: Limited/Not documented (graceful fallback)
 */

/**
 * Earn product type - unified across exchanges
 */
export type EarnProductType = "FLEXIBLE" | "LOCKED";

/**
 * Earn product status
 */
export type EarnStatus = "SUBSCRIBABLE" | "UNSUBSCRIBABLE" | "SOLD_OUT" | "PAUSED";

/**
 * Earn product - available for subscription
 * Unified interface across all exchanges
 */
export interface EarnProduct {
  /** Product ID on exchange */
  productId: string;
  /** Exchange */
  exchange: AllExchangeId;
  ...
  // Interest rates
  /** Base annual percentage yield (e.g., 5.5 for 5.5%) */
  apy: number;
  ...
  maxAmount?: number;
  /** Remaining quota for subscription */
  remainingQuota?: number;
  
  // For locked products
  /** Lock duration in days (for LOCKED type) */
  lockDuration?: number;
  /** Start time for locked product */
  startTime?: Date;
  /** End/redemption time for locked product */
  endTime?: Date;
  /** Can redeem early */
...
/**
 * Active earn position - user's subscribed position
 */
export interface EarnPosition {
  ...
  principal: number;
  /** Accumulated interest (not yet credited) */
  pendingInterest: number;
  /** Total interest earned (historical) */
  totalInterest: number;
  /** Current value (principal + pending interest) */
  totalValue: number;
  
  // Interest
  /** Annual percentage yield at time of subscription */
  apy: number;
  /** Next interest credit time */
  nextInterestTime?: Date;
  /** Last interest credit time */
  lastInterestTime?: Date;
  
  // For locked positions
  /** Subscription time */
  subscribeTime: Date;
  /** Maturity/redemption time (for LOCKED) */
  maturityTime?: Date;
...
/**
 * Earn account summary - aggregated earn positions
 */
export interface EarnAccount {
  /** Exchange */
  exchange: AllExchangeId;
  /** Total principal across all positions */
  totalPrincipal: number;
  /** Total pending interest */
  totalPendingInterest: number;
  /** Total interest earned historically */
  totalInterestEarned: number;
  /** Total current value */
  totalValue: number;
  /** Total in flexible savings */
  totalFlexible: number;
  /** Total in locked savings */
  totalLocked: number;
...
/**
 * Subscribe to earn product params
 */
export interface SubscribeEarnParams {
  /** Product ID */
  productId: string;
  /** Product type */
  type: EarnProductType;
  /** Amount to subscribe */
  amount: number;
  /** Auto-renew for locked products */
  autoRenew?: boolean;
}

/**
 * Redeem from earn params
 */
export interface RedeemEarnParams {
  /** Position ID */
  positionId: string;
  /** Product type */
  type: EarnProductType;
  /** Amount to redeem (partial redemption) */
  amount?: number;
  /** Force early redemption (may have penalty) */
...
/**
 * Earn products list with metadata
 */
export interface EarnProductsList {
  exchange: AllExchangeId;
...
export interface EarnHistoryItem {
  ...
  asset: string;
  type: EarnProductType;
  operation: "SUBSCRIBE" | "REDEEM" | "INTEREST_CREDIT" | "AUTO_RENEW";
  amount: number;
  interest?: number;
  timestamp: Date;
