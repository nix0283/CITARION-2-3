...
export class BinanceClient extends BaseExchangeClient {
  ...
  /**
   * Binance supports Simple Earn (Flexible + Locked)
   */
   ...
   */
  async getEarnProducts(
    asset?: string,
    type?: import("./types").EarnProductType
  ): Promise<import("./types").EarnProductsList> {
    const products: import("./types").EarnProduct[] = [];

    // Fetch flexible products
    if (!type || type === "FLEXIBLE") {
      try {
        const flexibleData = await this.signedRequest("GET", "/sapi/v1/simple-earn/flexible/list", {
          asset,
          size: 100,
        }) as {
          rows?: Array<{
        ...
        if (flexibleData.rows) {
          for (const row of flexibleData.rows) {
            products.push({
              ...
              minAmount: parseFloat(row.minPurchaseAmount),
              maxAmount: row.maxPurchaseAmount ? parseFloat(row.maxPurchaseAmount) : undefined,
              remainingQuota: row.remainingQuota ? parseFloat(row.remainingQuota) : undefined,
              status: this.mapEarnStatus(row.status),
              timestamp: new Date(),
            });
          }
        }
      } catch (error) {
        console.error("[Binance] Error fetching flexible products:", error);
      }
    }

    // Fetch locked products
    if (!type || type === "LOCKED") {
      try {
        const lockedData = await this.signedRequest("GET", "/sapi/v1/simple-earn/locked/list", {
          asset,
          size: 100,
        }) as {
          rows?: Array<{
        ...
        console.error("[Binance] Error fetching locked products:", error);
      }
    }
  ...
  async getEarnPositions(asset?: string): Promise<import("./types").EarnAccount> {
    const positions: import("./types").EarnPosition[] = [];
    let totalFlexible = 0;
    let totalLocked = 0;

    // Fetch flexible positions
    try {
      const flexibleData = await this.signedRequest("GET", "/sapi/v1/simple-earn/flexible/position", {
        asset,
        size: 100,
      }) as {
        rows?: Array<{
      ...
      if (flexibleData.rows) {
        for (const row of flexibleData.rows) {
          ...
          const pendingInterest = parseFloat(row.totalAmount) - principal;
          ...
          positions.push({
            ...
            apy: parseFloat(row.apy),
            subscribeTime: new Date(row.subscribeTime),
            canRedeem: row.status === "PURCHASE_SUCCESS",
            timestamp: new Date(),
          });

          totalFlexible += parseFloat(row.totalAmount);
        }
      }
    } catch (error) {
      console.error("[Binance] Error fetching flexible positions:", error);
    }

    // Fetch locked positions
    try {
      const lockedData = await this.signedRequest("GET", "/sapi/v1/simple-earn/locked/position", {
        asset,
        size: 100,
      }) as {
        rows?: Array<{
      ...
      if (lockedData.rows) {
        for (const row of lockedData.rows) {
          ...
          positions.push({
            ...
            asset: row.asset,
            principal,
            pendingInterest: 0, // Calculated at maturity
            totalInterest: parseFloat(row.totalInterest),
            totalValue: principal + parseFloat(row.totalInterest),
            apy: parseFloat(row.apy),
            subscribeTime: new Date(row.subscribeTime),
            maturityTime: row.maturityTime ? new Date(row.maturityTime) : undefined,
            lockDuration: row.duration,
            canRedeem: row.status === "MATURITY",
            autoRenew: row.autoRenew,
            timestamp: new Date(),
          });

          totalLocked += principal + parseFloat(row.totalInterest);
        }
      }
    } catch (error) {
      console.error("[Binance] Error fetching locked positions:", error);
    }

    const totalPrincipal = positions.reduce((sum, p) => sum + p.principal, 0);
    const totalPendingInterest = positions.reduce((sum, p) => sum + p.pendingInterest, 0);
    const totalInterestEarned = positions.reduce((sum, p) => sum + p.totalInterest, 0);
    ...
    return {
      ...
      totalInterestEarned,
      totalValue: totalPrincipal + totalPendingInterest + totalInterestEarned,
      totalFlexible,
      totalLocked,
  ...
  async subscribeEarn(params: import("./types").SubscribeEarnParams): Promise<import("./types").EarnResult> {
    try {
      const endpoint = params.type === "FLEXIBLE"
      ...
      if (params.type === "FLEXIBLE") {
        ...
        requestParams.projectId = params.productId;
        if (params.autoRenew !== undefined) {
          requestParams.autoRenew = params.autoRenew;
        }
      }

      const data = await this.signedRequest("POST", endpoint, requestParams) as {
        purchaseId?: string;
        success?: boolean;
      };
  ...
  async redeemEarn(params: import("./types").RedeemEarnParams): Promise<import("./types").EarnResult> {
    try {
      ...
      if (params.earlyRedemption && params.type === "LOCKED") {
        requestParams.destAccount = "SPOT";
      }
   ...
   * Get earn history
   */
  async getEarnHistory(limit: number = 100): Promise<import("./types").EarnHistoryItem[]> {
    const history: import("./types").EarnHistoryItem[] = [];

    // Fetch flexible history
    try {
      const data = await this.signedRequest("GET", "/sapi/v1/simple-earn/flexible/history/subscriptionRecord", {
        size: limit,
      }) as {
        rows?: Array<{
          id: string;
      ...
      if (data.rows) {
        for (const row of data.rows) {
          history.push({
            ...
            asset: row.asset,
            type: "FLEXIBLE",
            operation: this.mapHistoryOperation(row.type),
      ...
      console.error("[Binance] Error fetching flexible history:", error);
    }

    return history.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime()).slice(0, limit);
  }

  private mapEarnStatus(status: string): import("./types").EarnStatus {
    const statusMap: Record<string, import("./types").EarnStatus> = {
      PURCHASE_SUCCESS: "SUBSCRIBABLE",
      SUBSCRIBABLE: "SUBSCRIBABLE",
      UNSUBSCRIBABLE: "UNSUBSCRIBABLE",
      SOLD_OUT: "SOLD_OUT",
      PAUSED: "PAUSED",
    };
    return statusMap[status] || "UNSUBSCRIBABLE";
  }

  private mapHistoryOperation(type: string): "SUBSCRIBE" | "REDEEM" | "INTEREST_CREDIT" | "AUTO_RENEW" {
    const opMap: Record<string, "SUBSCRIBE" | "REDEEM" | "INTEREST_CREDIT" | "AUTO_RENEW"> = {
      SUBSCRIPTION: "SUBSCRIBE",
      REDEMPTION: "REDEEM",
      INTEREST: "INTEREST_CREDIT",
      AUTO_RENEW: "AUTO_RENEW",
    };
    return opMap[type] || "SUBSCRIBE";
  }

  private mapHistoryStatus(status: string): "PENDING" | "SUCCESS" | "FAILED" {
